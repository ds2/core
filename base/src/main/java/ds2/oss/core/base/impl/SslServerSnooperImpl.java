...
public class SslServerSnooperImpl implements SslServerSnooper {
    ...
    private static final Logger LOG = LoggerFactory
        .getLogger(SslServerSnooperImpl.class);
    /**
    ...
    @Override
    public X509Certificate[] getServerCertificates(final String hostname,
        final int port) {
        ...
        try {
            ...
            try (SSLSocket socket =
                (SSLSocket) sslsf.createSocket(hostname, port)) {
     ...
     */
    private static void writeTempKeystore(final String aliasHeader,
        final X509Certificate[] certs, final char[] pw) {
        ...
        try {
            ...
            final File tmpFile =
                File.createTempFile("ks-" + aliasHeader, ".jks", tmpDir);
            ...
            try (FileOutputStream fos = new FileOutputStream(tmpFile)) {
                ks.store(fos, pw);
                LOG.info("Server certificates are written to {}",
    ...
    private static KeyStore getCurrentKeystore() {
        ...
        try {
            ...
            if (javaHome != null) {
                final File javaHomeSec =
                    new File(javaHome + PATHSEP + "lib" + PATHSEP + "security");
            ...
            LOG.error("Error when loading a default instance of the keystore!",
                e);
        }
        return rc;
    }
     ...
     */
    private static void loadLocalKeystore(final KeyStore rc, final char[] ksPw,
        final File javaHomeKs) {
        try (FileInputStream fis = new FileInputStream(javaHomeKs)) {
            rc.load(fis, ksPw);
        } catch (final NoSuchAlgorithmException e) {
            LOG.error("An algorithm error occurred!", e);
        } catch (final CertificateException e) {
            LOG.error("A certificate error occurred on reading the keystore!",
                e);
        } catch (final IOException e) {
